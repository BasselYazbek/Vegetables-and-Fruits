...
const Discounts = ({ name, description, imagePath, price, discountedPrice, onEdit, onDelete, isAdmin, onToggleStock, isOutOfStock, fruitsMenu }) => {
  const [isEditingPrice, setIsEditingPrice] = useState(false);
  const [newPrice, setNewPrice] = useState(price);
  ...
  const handleSetDiscountedPrice = () => {
    ...
    if (discountedPriceInput !== null) {
      ...
      if (!isNaN(discountedPriceValue) && discountedPriceValue >= 0) {
        // Call the onEdit function to update the item's discounted price
        onEdit({ name, description, price, imagePath, discountedPrice: discountedPriceValue });
  ...
  const handleSavePrice = () => {
    ...
    // Call the onEdit function to update the item's price
    onEdit({ name, description, price: parseFloat(newPrice), imagePath, discountedPrice });
